...
def extract_tuple(biz, user, review):
	...
	for biz_json in biz_jsons:
		try:
			...
			cc = int(biz['attributes']['Accepts Credit Cards'])
			biz_dict[biz["business_id"]] = [biz["stars"], biz["review_count"], cc]
	...
	for review in review_set:
		...
		#FORMAT: BIZ_AVG_STAR, USER_AVG_RATING, RATING
		rating, review_count, credit_card = biz_dict[biz]
		triple = [rating, review_count, credit_card, user_dict[user], star]
...
if __name__ == "__main__":
	"""
	Usage: python3 create_triple_matrix.py --biz data/yelp_academic_dataset_business.json --review data/yelp_academic_dataset_review.json --user data/yelp_academic_dataset_user.json 
